					...
					<div class="user_details">
						...
						<div class="float-right">
							<div class="media">
								...
								<div class="d-flex">
									<img src="/blog/util/img/blog/user-img.jpg" alt="">
								</div>
							</div>
						</div>
					</div>
					...
					<c:forEach var="comment" items="${comments}">
						...
						<div class="comment-list" id="comment-id-${comment.id}">
							...
							<div class="reply-btn">
								...
								<button id="replyForm${comment.id}" onClick="replyForm(${comment.id},${sessionScope.user.id})" class="btn-reply text-uppercase" value ="a" style="display: inline-block; float: right; margin-right: 10px;">댓글쓰기</button>
							</div>
...
<script>
	...
	function replyDiv(){
		var comment_form_inner = "<h4 style='margin-bottom:20px'>Leave a Reply</h4><form id='reply-submit'>";
	...
	function replyForm(comment_id,user_id) {
		...
		var comment_form_inner = "<h4 style='margin-bottom:20px'>Leave a Reply</h4><form id='reply-submit'>";
			comment_form_inner += "<input type='hidden' name='userId' value='"+user_id+"' />";
			comment_form_inner += "<input type='hidden' name='commentId' value='"+comment_id+"' /><div class='form-group'>";
			comment_form_inner += "<textarea style='height:60px' class='form-control mb-10' rows='2' name='content' id='replycontent' placeholder='Messege' required=' '>";
			comment_form_inner += "</textarea></div><button type='button' onClick='replyWrite()' class='primary-btn submit_btn'>Post Comment</button></form>";

		//<div class="comment-form" style="margin-top:0px;"></div>
		var comment_form = document.createElement("div"); //div 빈 박스 생성
		comment_form.className = "comment-form"; //div에 클래스 이름을 주고
		comment_form.tagName = "comment-form"+comment_id;
		comment_form.style = "margin-top:0px"; //div에 style을 준다.
		...
		$("#replyForm"+comment_id).val('b');
			...
			document.querySelector("#comment-form"+comment_id).remove();
		}
	}
</script>
</body>
